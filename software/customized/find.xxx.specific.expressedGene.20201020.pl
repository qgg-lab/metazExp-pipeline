...
$minMappedVolume, $alignInfo, $specifiedExptIdListFile,
$outputValidedGeneExpressList, $outputTmpList);

GetOptions(
        'experimentFile=s'=>\$experimentFile,
	'minMappedVolume=s'=>\$minMappedVolume,
	'specifiedExptIdListFile=s'=>\$specifiedExptIdListFile,
	'alignInfo=s'=>\$alignInfo,
	...
	'outputTmpList=s'=>\$outputTmpList,
);


my ($line, $i, @nameField, @valueField, %tmpHash);
my (%validExptId);
# 从alignInfo文件中读取比对量，然后用minMappedVolume过滤,筛选出参与组织特异性的experiment
open FF, "<$alignInfo";
$line = <FF>;
chomp($line);
...
while($line=<FF>){
	...
	# 如果比对量低于指定的值那么放弃该实验样本
	next if($tmpHash{"mappedBases"}<$minMappedVolume);
	$validExptId{$tmpHash{"Experiment"}} = 1;
}
close FF;

# 如果有指定参与组织特异性分析的实验列表，那么放弃之前的过滤
if(uc($specifiedExptIdListFile) ne "NA"){
	%validExptId = ();
	open FF, "<$specifiedExptIdListFile";
	while($line=<FF>){
		chomp($line);
		$validExptId{$line} = 1;
	}
...
while($line=<FF>){
	...
	next if(index($ignoreTypeList, $tmpHash{"Tissue"})>=0);

	# 如果该实验不在之前指定或者比对容量过滤范围内的实验，那么放弃
	next if(not exists($validExptId{$tmpHash{"Experiment"}}));
...
foreach $experimentId(@experimentId){
	...
	while($line=<FF>){
		...
		$attrName = $experimentInfoHref->{$experimentId}->{"Tissue"};
		# 将TPM值添加到基因下对应的组织类型中
...
open TMPLIST, ">$outputTmpList";
print WW join("\t", "pvalue0.01Rlt", "pvalue0.03Rlt", "pvalue0.05Rlt", "geneId", "High/Low", "tissue", "avgExpTpm", "experimentNum", "1stTissue|avgTpm|exptNum|pvalue", "2ndTissue|avgTpm|exptNum|pvalue", "...") . "\n";
print TMPLIST join("\t", "pvalue0.01Rlt", "pvalue0.03Rlt", "pvalue0.05Rlt", "geneId", "High/Low", "tissue", "avgExpTpm|TpmList", "1stTissue|pvalue|avg(TPM)|TPMList", "2ndTissue|pvalue|avg(TPM)|TPMList", "...") . "\n";
...
###################################################################################################
	...
	my ($outputHighTpmListLine, $outputLowTpmListLine) =  ("", "");
	...
	$outputHighTpmListLine = join("\t", $geneId, "High", $maxExpressAttrName, $maxExpress . "|" . $firstValidedGeneToFieldtypeToTpmListHref->{$geneId}->{$maxExpressAttrName}->{"tpmList"});
...
#<SDTIN>;
		...
		$outputHighTpmListLine .= "\t" . $attrName . "|" . sprintf("%.3f", $pvalue) . "|" . $avgExpress . "|" . $firstValidedGeneToFieldtypeToTpmListHref->{$geneId}->{$attrName}->{"tpmList"};
		#1stTissue|pvalue|avg(TPM)|TPMList
	}


	# 输出最高表达组织和其它组织的显著差异水平
	print WW join("\t", $pvalue001Flag, $pvalue003Flag, $pvalue005Flag, $outputHighLine) . "\n";
	print TMPLIST join("\t", $pvalue001Flag, $pvalue003Flag, $pvalue005Flag, $outputHighTpmListLine) . "\n";
	...
	$outputLowTpmListLine = join("\t", $geneId, "Low", $minExpressAttrName, $minExpress . "|" . $firstValidedGeneToFieldtypeToTpmListHref->{$geneId}->{$minExpressAttrName}->{"tpmList"});
	foreach $attrName(@attrName){
...
EOF
		...
		$outputLowTpmListLine .= "\t" . $attrName . "|" . sprintf("%.3f", $pvalue) . "|" . $avgExpress . "|" . $firstValidedGeneToFieldtypeToTpmListHref->{$geneId}->{$attrName}->{"tpmList"};
	}

	# 输出最地表达组织和其它组织的显著差异水平
	print WW join("\t", $pvalue001Flag, $pvalue003Flag, $pvalue005Flag, $outputLowLine) . "\n";	
	print TMPLIST join("\t", $pvalue001Flag, $pvalue003Flag, $pvalue005Flag, $outputLowTpmListLine) . "\n";
