...
GetOptions(
	...
	'outputPsiListByTissue=s'=>\$outputPsiListByTissue,
);
...
foreach $psiFile(@psiFile){
	...
	while($line=<FF>){
		...
		if(not(exists($asToFieldTypeToPsiHref->{$asId}->{$fieldType}))){
			$asToFieldTypeToPsiHref->{$asId}->{$fieldType} = sprintf("%.5f", $psi);
		}else{
			$asToFieldTypeToPsiHref->{$asId}->{$fieldType} .= "," . sprintf("%.5f", $psi);
		}
	}
...
open PSILIST, ">$outputPsiListByTissue";
print WW join("\t", "pvalue0.01", "pvalue0.03", "pvalue0.03", "ASID", "High/Low", "$specifiedField=PSI|exptNum|minDltPsi", "1stTissue|avgPSI|exptNum|pvalue", "2ndTissue|avgPSI|exptNum|pvalue", "……") . "\n";
print PSILIST join("\t", "pvalue0.01", "pvalue0.03", "pvalue0.03", "ASID", "High/Low", "$specifiedField=AVG(PSI)|PSIList", "1stTissue|pvalue|avg(PSI)|PSIlist", "2ndTissue|pvalue|avg(PSI)|PSIlist", "……") . "\n";
...
########## 用t检验检查最大值的组织特异性 ###############
	...
	if($highEnoughTag == 1){
		...
		print PSILIST join("\t", $rltListBy3Cutoff, $asId, "High", $maxFieldType . "=" . sprintf("%.3f", $validFieldTypeToAvgPsiHref->{$maxFieldType}) . "|" . $asToFieldTypeToPsiHref->{$asId}->{$maxFieldType});
		...
		for(my $m=0; $m<=$#fieldType;  $m++){
			...
			print PSILIST "\t" . join("=", $fieldType, $pvalue . "|" . sprintf("%.5f", $validFieldTypeToAvgPsiHref->{$fieldType}) . "|" . $asToFieldTypeToPsiHref->{$asId}->{$fieldType});
		}
		print WW "\n";
		print PSILIST "\n";
	}
...
######### 用t检验检查最小值的组织特异性 #################
	...
	if($lowEnoughTag == 1){
		...
		print PSILIST join("\t", $rltListBy3Cutoff, $asId, "Low", $minFieldType . "=" . sprintf("%.3f", $validFieldTypeToAvgPsiHref->{$minFieldType}) . "|" . $asToFieldTypeToPsiHref->{$asId}->{$minFieldType});
