...
for(my $i=0; $i<=$#group; $i++){
	for(my $j=$i+1; $j<=$#group; $j++){
		...
		# = 3 finalOutputList
		$cmdFinalOutputPara[$cmdId] .= $jobId . "." . $taxonId . "." . $group[$i] . "." . $group[$j] . ".diff.expr.gene.by.tpm.tsv" . "_";
		# 命令行编号改变
		$cmdId++;
...
my $cmd;
my $launchShellFile = $outputDir . "/$jobId.$taxonId.launch.perlProgam.with.tpm.cmd.sh";
...
# 从比较结果中读取foldChange和pvalue
for(my $i=0; $i<=$#group; $i++){

  for(my $j=$i+1; $j<=$#group; $j++){
	
	#print join("\t", $group[$i], $group[$j]) . "\n";
	my $finalOutput = "$outputDir/$jobId.$taxonId.$group[$i].$group[$j].diff.expr.gene.by.tpm.tsv";

	# 结合比较信息提取差异表达的gene存入差异表达统一hash $geneGroupCompareHref
	my @field;
	open FF, "<$finalOutput";
	# gene_Id         control         treatment              foldChange              pvalue
	# ENSRNA049471065 0.12,0.10,0.0   76.4,9.04,6.07,90.12   15951.6176988758        0.00290610522289939
	<FF>;
	while($line = <FF>){
		chomp($line);
		@field = ();
		...
		$geneGroupCompareHref->{$geneId}->{$groupX . "/" . $groupY}->{"foldChange"} = $field[3];
		if($field[3] != 0){
			$geneGroupCompareHref->{$geneId}->{$groupY . "/" . $groupX}->{"foldChange"} = 1/$field[3];
		}else{
			$geneGroupCompareHref->{$geneId}->{$groupY . "/" . $groupX}->{"foldChange"} = "Inf";
		}
...
open WW, ">$outputDir/$jobId.$taxonId.final.specific.expr.gene.list.by.tpm.tsv";
# 对所有基因执行如下操作：
#     逐一检查每个group是否为特异
