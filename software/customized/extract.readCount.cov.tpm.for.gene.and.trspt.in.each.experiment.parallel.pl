...
# 获得尾缀，如/mnt/research/qgg/liujind1/workAS1/7227/013/xaa
my @tt = split(/\//, $exptIdListFile);
my $tailFix = $tt[$#tt];
...
foreach $exptId(@exptId){
	...
	open WW, ">file_position.txt.$tailFix";
	print WW $exptId . " " . $assemblyFile;
	close WW;

	# 执行prepDE程序，获得gene和trspt上read数量
	$cmd = "python $prepDE -i file_position.txt.$tailFix -g gene.count.tsv.$tailFix -t trspt.count.tsv.$tailFix -l $readLen";
	system($cmd);

	# 打开gene.count.tsv，将gene上的read数量累加进hash
	system("dos2unix gene.count.tsv.$tailFix");
	open FF, "<gene.count.tsv.$tailFix";
	<FF>;
	while($line = <FF>){
		chomp($line);
	...
	system("dos2unix trspt.count.tsv.$tailFix");
	open FF, "<trspt.count.tsv.$tailFix";
	<FF>;
	while($line = <FF>){
		chomp($line);
