...
$statTest, $changedPsi, $pvalue, $minReadCoverage,
...
#print join("\t", $taxonId, $jobId, $exptDir, $exptGroupInfo, $changedPsi, $pvalue, $minReplicateNum, $minGroupNum, $outputDir) . "\n";
...
# 将AS在每个experiment中PSI按照group进行分类，登记到每个AS的hash中
my (@exptId, $exptId, @psi, $asId, $psi, @field, $IJC_SAMPLE, $SJC_SAMPLE, $IncFormLen, $SkipFormLen, $experimentId);
my ($psiFile);
my (%asPsiListInGroup, $asPsiListInGroupHref);
$asPsiListInGroupHref = \%asPsiListInGroup;
foreach $group(@group){
	@exptId = ();
	@exptId = split(/,/, $groupToExptIdList{$group});
	foreach $exptId(@exptId){
		$psiFile = $exptDir . "/" . $exptId;
		open FF, "<$psiFile";
		# ASID    		IJC_SAMPLE_1    SJC_SAMPLE_1    IncFormLen      SkipFormLen     Experiment
		# OSJGA3SS0000000001    1       	16      	104     	100     	SRX1203046
		<FF>;
		while($line = <FF>){
			chomp($line);
			($asId, $IJC_SAMPLE, $SJC_SAMPLE, $IncFormLen, $SkipFormLen, $experimentId) = split(/\t/, $line);
			# read覆盖数量不达标，那么放弃该AS参与统计
			next if($IJC_SAMPLE + $SJC_SAMPLE < $minReadCoverage);
			# 计算psi
			$psi = ($IJC_SAMPLE/$IncFormLen)/($IJC_SAMPLE/$IncFormLen + $SJC_SAMPLE/$SkipFormLen);
			$asPsiListInGroupHref->{$asId}->{$group} .= $psi . ",";
		}
		close FF;
	}
}

# 将as的PSI列表按照group分组输出到文件，以便于后续R调用
my (@asId, $outputLine, $status, @psi);
@asId = keys(%asPsiListInGroup);
my $psiByGroupFile = $outputDir . "/$jobId.$taxonId.psi.list.in.group.tsv";

open WW, ">$psiByGroupFile";
print WW join("\t", "asId", @group) . "\n";
foreach $asId(@asId){
	
	# 初始化当前asId的psi采集状态为成功
	$status = 1;

	# 初始化输出行
	$outputLine = $asId;

	# 采集当前asId下所有group内的psi列表
	for(my $i=0; $i<=$#group; $i++){
		# 如果当前group下没有搜集到PSI列表，那么放弃整个asId不参与差异统计
		if(not(exists($asPsiListInGroupHref->{$asId}->{$group[$i]}))){
			$status = 0;
			last;
		}		
		
		# 去掉末尾的","
		$asPsiListInGroupHref->{$asId}->{$group[$i]} = substr($asPsiListInGroupHref->{$asId}->{$group[$i]}, 0, length($asPsiListInGroupHref->{$asId}->{$group[$i]}) - 1);

		# 统计psi个数
		@psi = ();
		@psi = split(/,/, $asPsiListInGroupHref->{$asId}->{$group[$i]});
		if($#psi + 1 < 2){
			$status = 0;
			last;
		}
		...
		# 
		$outputLine .= "\t" . $asPsiListInGroupHref->{$asId}->{$group[$i]};
	}

	# 检测当前asId下所有的group是否都采集成功：不全部成功则放弃该asId
	next if($status == 0);
...
# 调用R处理输出文件：$jobId.$taxonId.psi.list.in.group.tsv
my $tmpRlt = $outputDir . "/$jobId.$taxonId.diff.splicing.as.by.psi.tsv";
my $R=Statistics::R->new(shared => 1);
my $cmd=<<EOF;
df<-read.table(file="$psiByGroupFile", sep="\\t", quote ="", header=T)
asId <- df\$asId
psi<-df[,-1]
groupNum = dim(psi)[2]
groupName=colnames(psi)
rlt <- data.frame(a=c(1:length(asId)))
for(i in c(1:groupNum )){
  if(i==groupNum)
    break
  for(j in c((i+1):groupNum)){
    compareName <- paste("changedPsi", groupName[i], groupName[j], sep="/")
    rlt[,compareName] = apply(psi, 1, function(x) mean(as.numeric(unlist(strsplit(x[j], ",")))) - mean(as.numeric(unlist(strsplit(x[i], ",")))))
  }
}
rlt<-rlt[,-1]
...
for(i in c(1:groupNum )){
   ...
   for(j in c((i+1):groupNum)){
	compareName <- paste("adjPvalue", groupName[i], groupName[j], sep="/")
        rlt[,compareName] = p.adjust(apply(psi, 1, function(x) t.test(as.numeric(unlist(strsplit(x[i], ","))),as.numeric(unlist(strsplit(x[j], ","))), alternative='two.sided', paired = F)\$p.value), method="fdr")
   }
}
rownames(rlt)=geneId
...
# 开始筛选
# geneId spGroup   spDirection   meanTpm   minPvalue maxPvalue minFC maxFC psiListInAllGroup
# Os01   root      high          12.3456   0.01      0.04      2     4     root:10.3,12.9,3.8;leaf:12.4,23.8,48.5;
my (@tpm, $meanTpm, $psiListInAllGroups, $tmpMinFC, $tmpMaxFC);
my ($line, @nameField, @valueField, $asId);
...
print WW join("\t", "asId", "spGroup", "spDirection", "meanTpm", "minPvalue", "maxPvalue", "minFC", "maxFC", "psiListInAllGroup") . "\n";
open FF, "<$tmpRlt";
$line = <FF>;
...
while($line=<FF>){
	...
	foreach $groupX(@group){
		...
		($minPvalue, $maxPvalue, $minFC, $maxFC, $spDirection, $giveUp, $psiListInAllGroups) = (Inf, -Inf, Inf, -Inf, "-", "no", "");
		...
		if(($minFC >= $foldChange or $maxFC <= 1/$foldChange) and $maxPvalue <= $pvalue){
			...
			# 求当前 groupX的tpm均值			
			@tpm = ();
			@tpm = split(/,/,$asPsiListInGroupHref->{$asId}->{$groupX});
			$meanTpm = sum(@tpm)/($#tpm+1);

			# 将所有group的tpm都列出来
			$psiListInAllGroups = "";
			foreach $groupY(@group){
				$psiListInAllGroups .= $groupY . ":" . $asPsiListInGroupHref->{$asId}->{$groupY} . ";";
			}
			print WW join("\t", $asId, $groupX, $spDirection, sprintf("%.4f", $meanTpm), $minPvalue, $maxPvalue, $minFC, $maxFC, substr($psiListInAllGroups, 0, length($psiListInAllGroups)-1) ) . "\n";
		}
	}
}
close FF;
