...
my ($expId, $runIdList, $fastqOutputDir, $cDNAdb, $genomeDb, $psiOutputDir, $logOutputDir);
my ($gtfFile, $rmatsPy, $threadNum, $cmd, $switchPsi, $switchAssemble);
...
if($#keyPhredScore > 0 or $#keyLibraryType > 0 or $#keyLayout > 0 or $dumpRunDataFailedNum > 0){
	exit;
}
