...
if($#ARGV < 0){
	print "perl $0 \\\n" . 
		"\t\t--mergedAnnoGtf  combined.gtf \\\n" .
		"\t\t--mergeLabel     combine \\\n" .
		"\t\t--anno1Gtf       ensembl.annotation.gtf \\\n" . 
		"\t\t--anno1Label     Ensembl \\\n" .
		"\t\t--anno2Gtf       refSeq.annotation.with_ensemblSeqId.gtf \\\n" . 
		"\t\t--anno2Label     Refseq \\\n" . 
		"\t\t--xvfb-run       /usr/bin/xvfb-run  \\\n" .
		"\t\t--Rscript        /opt/software/R/3.5.1-foss-2018a-X11-20180131/bin/Rscript \\\n" . 
		"\t\t--drawVennRscript /mnt/home/liujind1/software/customized/" . 
			"draw.vennDiagram.for.three.sets.R \\\n" . 
		"\t\t--drawBarRscript /mnt/home/liujind1/software/customized/" .
			"draw.barDiagram.for.three.sets.R \\\n" .
		"\t\t--drawDensityRscript /mnt/home/liujind1/software/customized/" . 
			"draw.densityDiagram.for.three.sets.R  \\\n" .
		"\t\t--densityExonMinLen 1 \\\n" .
		"\t\t--densityExonMaxLen 200 \\\n" .
		"\t\t--densityTranscriptMinLen 100 \\\n" .
		"\t\t--densityTranscriptMaxLen 5000 \\\n" .

		"\t\t--outputStaTbl    stat.tsv \\\n" .
		...
		"\t\t--outputExonNumEnsRefMergBar     bar.of.trspt.with.exonNum.bar.png \\\n" .
		"\t\t--outputImagType png \\\n" . 
...
my($mergedAnnoGtf, $anno1Gtf, $anno2Gtf, 
	$mergeLabel, $anno1Label, $anno2Label,
	$xvfb_run,
...
my (%anno1TrsptIdToConcatExonCoord, %anno2TrsptIdToConcatExonCoord, %mergeTrsptIdToConcatExonCoord);
...
my (@anno1AcceptorCoord, @anno2AcceptorCoord, @mergeAcceptorCoord);
my (%anno1TranscriptNum, %anno2TranscriptNum, %mergeTranscriptNum);
my (%anno1TrsptIdToExonNum, %anno2TrsptIdToExonNum, %mergeTrsptIdToExonNum);
...
my $cmd = $Rscript . " " . $drawVennRscript . " " .
	...
	" -b $anno2Label" .
	" -C $tmpOutputDir/$mergeLabel.transcript.concat.exon.coord.txt" .
	" -c $mergeLabel" .
...
my $cmd = $Rscript . " " . $drawVennRscript . " " .
	...
	" -b $anno2Label" .
	" -C $tmpOutputDir/$mergeLabel.uniq.exon.coord.txt" .
	" -c $mergeLabel" .
...
my $cmd = $Rscript . " " . $drawVennRscript . " " .
	...
	" -b $anno2Label" .
	" -C $tmpOutputDir/$mergeLabel.uniq.donor.coord.txt" .
	" -c $mergeLabel" .
...
my $cmd = $Rscript . " " . $drawVennRscript . " " .
	...
	" -b $anno2Label" .
	" -C $tmpOutputDir/$mergeLabel.uniq.acceptor.coord.txt" .
	" -c $mergeLabel" .
