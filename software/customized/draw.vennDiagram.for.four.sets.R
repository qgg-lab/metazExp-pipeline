...
args <- getopt(command)
if(is.null(args$aSetFile) || is.null(args$bSetFile) || is.null(args$cSetFile) || is.null(args$dSetFile) || is.null(args$imageFile) || is.null(args$mageType)){
...
venn.diagram(x=list(first1=A[,1], second2=B[,1], third3=C[,1], fourth=D[,1]),category.names = c(args$aSetLabel, args$bSetLabel, args$cSetLabel, args$dSetLabel), ,col="transparent",fill=c("tomato2","skyblue3","purple2", "seagreen3"),alpha=0.50,cex=c(1.2, 1, 1.2, 1, 1, 0.7, 1, 1, 1.2, 1, 1, 1, 1, 1.2, 1),fontfamily="serif",fontface="bold",cat.col=c("tomato2","skyblue3","purple2","seagreen3"),cat.cex = 1.2, cat.dist = c(0.2,0.2,0.09,0.09),cat.fontfamily="serif", margin = 0.05, filename = args$imageFile, imagetype = args$mageType)
