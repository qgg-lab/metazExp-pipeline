...
$outputValidedGeneExpressList, $outputTpmList);
...
my ($fieldNameList, $fieldValueList, @fieldName, @fieldValue, $trsptId, $attrName);
...
foreach $experimentId(@experimentId){
	$abundanceFile = $abundanceDir . "/" . $experimentId . "/transcriptomeByStringtie.gtf";
	my $cmd = "grep -P \"\\ttranscript\\t\" " . $abundanceFile;
	my $rltTxt = `$cmd`;
	# 1       StringTie       transcript      3631    5899    1000    +       .       gene_id "DRX007662.1"; transcript_id "DRX007662.1.1"; reference_id "AT1G01010.1"; ref_gene_id "AT1G01010"; ref_gene_name "NAC001"; cov "10.644747"; FPKM "4.932029"; TPM "7.918104"; 
	my @line = ();
	@line = split(/\n/, $rltTxt);
	foreach $line(@line){
		if($line=~/.*; reference_id "(.*?)"; .*?cov "(.*)"; FPKM "(.*)"; TPM "(.*?)";/){
			($geneId, $cov, $fpkm, $tpm) = ($1, $2, $3, $4);
...
print TPMLIST join("\t", "pvalue0.01Rlt", "pvalue0.03Rlt", "pvalue0.05Rlt", "geneId", "High/Low", "tissue", "avgExpTpm|TpmList", "1stTissue|pvalue|avgTpm|TpmList", "2ndTissue|pvalue|avgTpm|TpmList", "...") . "\n";
...
#<SDTIN>;
	...
	print TPMLIST join("\t", $pvalue001Flag, $pvalue003Flag, $pvalue005Flag, $outputHighTpmListLine) . "\n";
...
EOF
		...
		$outputLowTpmListLine .= "\t" . $attrName . "|" . sprintf("%.5f", $pvalue) . "|" . $avgExpress . "|" . $firstValidedGeneToFieldtypeToTpmListHref->{$geneId}->{$attrName}->{"tpmList"};

	}

	# 输出最地表达组织和其它组织的显著差异水平
	print WW join("\t", $pvalue001Flag, $pvalue003Flag, $pvalue005Flag, $outputLowLine) . "\n";
	print TPMLIST join("\t", $pvalue001Flag, $pvalue003Flag, $pvalue005Flag, $outputLowTpmListLine) . "\n";
