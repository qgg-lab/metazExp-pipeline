...
my ($trRlt, $spRlt, $trFasta, $spFasta, $finalGtf, $ensemblGtf, $outputGeneAnno);
...
my (%annoTrsptToGeneId, %annoGeneToSymbol, @field, $geneId, $trsptId);
open FF, "<$finalGtf";
while($line=<FF>){
	# D       ena     transcript      2       1711    .       +       .       gene_id "CGB_D0010W"; transcript_id "CGB_D0010W"; gene_source "ena"; gene_biotype "pseudogene"; transcript_source "ena"; transcript_biotype "pseudogene";
	chomp($line);
	@field = split(/\t/, $line);
	next if($field[2] ne "transcript");
	($geneId, $trsptId) = ("", "");
	&getId($field[8], \$geneId, \$trsptId);
	$annoTrsptToGeneId{$trsptId} = $geneId;
	# $annoGeneToSymbol{$2} = $3;	
}
close FF;
...
while($line=<FF>){
	...
	($geneId, $symbol) = ("", "");
	&getSymbol($field[8], \$geneId, \$symbol);
	if($symbol ne ""){
		$annoGeneToSymbol{$geneId} = $symbol;
	}
}
close FF;
# 读取tr注释的结果
...
sub getId{
	...
	my (@tt, $tt);
	# gene_id "CGB_D0010W"; transcript_id "CGB_D0010W"; gene_source "ena"; gene_biotype "pseudogene"; transcript_source "ena"; transcript_biotype "pseudogene";
	@tt = ();
	@tt = split(/;/, $attrString);
	foreach $tt(@tt){
		if($tt=~/gene_id "(.*?)"/){
