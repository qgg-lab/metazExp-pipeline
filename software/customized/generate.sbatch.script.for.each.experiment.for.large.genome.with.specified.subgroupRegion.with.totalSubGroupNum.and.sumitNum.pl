...
my ($subSeqIdListDir, $subSeqIdListPrefix, $subGtfDir, $subGtfPrefix, $callAsFromBam, $submitNum, $totalSubGroupNum);
...
GetOptions(
	...
	'totalSubGroupNum=s'=>\$totalSubGroupNum,
...
foreach $experimentId (@experimentId){
	...
	# submitNum totalSubGroupNum
	my ($reginBeg, $reginEnd, $spanSize);
	$spanSize = int($totalSubGroupNum/$submitNum);
	for($reginBeg=0; $reginBeg<$totalSubGroupNum; $reginBeg=$reginBeg+$spanSize){
		$reginEnd = $reginBeg + $spanSize - 1;
		if($reginEnd > $totalSubGroupNum - 1){
			$reginEnd = $totalSubGroupNum - 1;
		}
		$sbatchText.= "perl " . $callAsFromBam .
			...
			" --specifiedSubGroupRegion " . join("-", $reginBeg, $reginEnd) . " " .
...
# 收集分散在sub中的可变剪接事件
	$sbatchText.= "perl /mnt/home/liujind1/software/customized/gather.as.from.subgroupRegion.pl " .
		...
		" --outputSubPrefix sub " .
		" --seqInfoFile " . $alignmentFile .
		" --subGroupRegion " . join("-", 0, $totalSubGroupNum-1) .
		" 1>" . $psiOutputDir . "/" . $experimentId . "/log.o.txt " .
		" 2>" . $psiOutputDir . "/" . $experimentId . "/log.e.txt\n\n\n";
