...
foreach $experimentId (@experimentId){
	...
	next if(${$experiment{$experimentId}}{"Assemble"}==0);
	$sbatchText= "#!/bin/bash\n";	
	...
	$sbatchText.= "perl " . $psiScript . " --expId " . $experimentId . " --runIds " . ${$experiment{$experimentId}}{"RunList"} . " --genomeDb " . $genomeDb . " --cDNAdb " . $cDNAdb . " --gtfFile " . $gtfFile . " --maxIntronSize " . $maxIntronSize . " --minIntronSize " . $minIntronSize . " --rMats " . $rMATSscript . " --switchPsi " . $switchPsi . " --switchAssemble " . $switchAssemble . " --fastqOutputDir " . $fastqOutputDir . " --psiOutputDir " . $psiOutputDir . " --logOutputDir " . $logOutputDir . " --manualDownloadDir " . $manualDownloadDir . " --threadNum $cpusPerTask 1> " . $taskOutputDir . "/log.o." . $experimentId . ".cmd.sh 2> " . $taskOutputDir . "/log.e." . $experimentId . "\n\n\n";
