...
my ($taxonIdListFile, $sradb, $workDir, $outputExpTsvFile, $sqlite3, $minMillionSpot, $minBase);
...
foreach $taxonmy(@taxonmy){
	...
	open FF, "<" . $workDir . "/tmp.$taxonmy.experiment.sra.txt";
	while($line=<FF>){
		chomp($line);
	...
	$cmd = "rm -rf " . $workDir . "/tmp.$taxonmy.experiment.sra.txt";
	system($cmd);
