...
my (%trsptTogene, %geneToTrsptIdList, @geneId, $geneId, @trsptId, $trsptId, @field, $field, @attr, $attr, $line, $trsptVersion, $geneVersion);
...
while($line=<FF>){
	...
	@field = split(/\t/, $line);
	next if($#field!=8);
	($geneId, $trsptId, $geneVersion, $trsptVersion) = ("", "", "", "");
	@attr = ();
	@attr = split(/;/, $field[8]);
	foreach $attr(@attr){
		if($attr=~/gene_id "(.*?)"/){
