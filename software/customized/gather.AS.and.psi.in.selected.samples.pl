...
# 定义的totalAsA5ssHash用于登记一个experiment识别的可变剪接事件
my (%totalAsA5ssHash, %totalAsA3ssHash, %totalAsSeHash, %totalAsRiHash, %totalAsMxeHash);

# 定义catalogASxxx用于汇总所有experiment识别的可变剪接事件的目录
my (%catalogA5ss, %catalogA3ss, %catalogRi, %catalogMxe, %catalogSe);
...
while($line=<FF>){
	@fields = ();
	@fields = split(/\//, $line);
	$expId = $fields[$#fields-1];

	#register total AS and extract total AS num
	# totalAsA4ssHash, totalAsA3ssHash, totalAsSeHash, totalAsRiHash, totalAsMxeHash 是当前experiment下的total
	# 将当前experiment下的可变剪接事件清空
	%totalAsA5ssHash = ();
	%totalAsA3ssHash = ();
	%totalAsSeHash = ();
	%totalAsRiHash = ();
	...
	# 读取当前experiment下识别的可变剪接事件位置信息
	# 根据当前AS类型下的asId编号（如0，或者1_0）将该AS在对应的experiment中map情况登记到totalAsASxxx哈希中
	# 根据位置信息判断是否需要将其登记到catalog哈希中
	...
	while(my $line = <FA5SS>){
		...
		# 获得AS的编号，并将编号从数组@tmpAsField中剔除掉
		$asId = $tmpAsField[0];
		shift(@tmpAsField);
		# 根据AS位置信息生成全局唯一性的“位置型”编号
		$asPosition = join("#", @tmpAsField);
		# save into %totalA5ss
		$totalAsA5ssHash{$asId} = $asPosition;
		# 以全局唯一性的“位置型”编号为索引，将AS登记到全局哈希中
